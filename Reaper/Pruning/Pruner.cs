...
{
   public static IReadOnlySet<string> FlagForRemoval(IEnumerable<Entry> entries, long retentionSeconds, long nowSeconds)
   {
      var all = entries.ToList();

      var flagged = all.Where(e => nowSeconds - e.RefreshedAt > retentionSeconds).Select(e => e.Path).ToHashSet();
      ...
      flagged.RemoveWhere(path => protectedDirs.Contains(path) || Ancestors(path).Any(protectedDirs.Contains));
