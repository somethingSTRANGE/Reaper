...
public class PrunerTests
{
   ...
   {
      return new Entry(path, Now, Now - (days * Day), Size: 0);
   }
   ...
   {
      var entries = new[] { Aged("Stale/old1.txt", 30), Aged("Stale/old2.txt", 30) };
      var flagged = Pruner.FlagForRemoval(entries, SevenDays, Now);
      Assert.That(flagged, Is.EquivalentTo(["Stale/old1.txt", "Stale/old2.txt"]));
   }

   [Test]
   public void Retained_file_near_root_protects_deeper_descendants()
   {
      var entries = new[] { Aged("Foo/Bar/young.txt", 30), Aged("Foo/Bar/old.log", 30), Aged("Foo/other.txt", 2) };
      var flagged = Pruner.FlagForRemoval(entries, SevenDays, Now);
      Assert.That(flagged, Is.Empty);
   }
   ...
   {
      var entries = new[] { Aged("Foo/Bar/young.txt", 2), Aged("Foo/Bar/old.log", 30), Aged("Foo/other.txt", 30) };
      var flagged = Pruner.FlagForRemoval(entries, SevenDays, Now);
      Assert.That(flagged, Is.Empty);
   }
   ...
   {
      var entries = new[] { Aged("Foo/young.txt", 2), Aged("Foo/old.txt", 30) };
      var flagged = Pruner.FlagForRemoval(entries, SevenDays, Now);
      Assert.That(flagged, Is.Empty);
   }
   ...
   public void Retention_does_not_bleed_across_sibling_subtrees()
   {
      ...
      var flagged = Pruner.FlagForRemoval(entries, SevenDays, Now);
      Assert.That(flagged, Is.EquivalentTo(["StaleStuff/a.txt"]));
   }

   [Test]
   public void Stale_directory_entry_is_protected_by_retained_child()
   {
      var entries = new[] { Aged("Foo", 30), Aged("Foo/Bar", 30), Aged("Foo/Bar/young.txt", 2) };
      var flagged = Pruner.FlagForRemoval(entries, SevenDays, Now);
      Assert.That(flagged, Is.Empty);
   }
}
